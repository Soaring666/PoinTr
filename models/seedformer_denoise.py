...
def array2samples_distance(array1, array2):
    """
    array2samples_distance(x, y)表示的是cd损失中的第二项,即固定了y的顺序
    ...
    distances, min_idx = torch.min(distances, dim=1)
...
class SeedFormer_denoise(nn.Module):
    """
    ...
    # radius=1, up_factors=None, seed_factor=2, interpolate='three', attn_channel=True):
        ...
        """
        super(SeedFormer_denoise, self).__init__()
    ...
    def forward_decoder(self, feat, partial_cloud, patch_xyz, patch_feat):
        ...
        for i, layer in enumerate(self.up_layers):
            pcd, _ = layer(pcd, seed, seed_feat, K_prev)
            if i <= 1:
                pcd = self.denoise[i](pcd.permute(0, 2, 1).contiguous())
                pred_pcds.append(pcd)

...
'''
from diffusion_utils import UNet, UNet_new, PointNet2CloudCondition, FBNet
...
class DDPM_FBNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        """
        super(DDPM_FBNet, self).__init__()
        self.nn_model = FBNet()
    ...
    def forward(self, x, condition_xyz):
        """
        this method is used in training, so samples t and noise randomly
        input:
            x: noisy point (B, 512, 3)
            condition_xyz: seed point (B, 256, 3)
        """
        B, N, C = x.shape
    ...
    def sample(self, shape, condition_xyz):
        ...
        for i in range(self.n_T - 1, 0, -1):
            ...
            eps = self.nn_model(x_i, t_is, condition_xyz)
            x_i = (

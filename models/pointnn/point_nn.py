...
class FPS_kNN(nn.Module):
    ...
    def forward(self, xyz, x):
        ...
        lc_xyz = index_points(xyz, fps_idx)     #center points(B, 512, 3)
        lc_x = index_points(x, fps_idx)         #center points(B, 512, 72)

        # kNN
        knn_idx = knn_point(self.k_neighbors, xyz, lc_xyz)
        knn_xyz = index_points(xyz, knn_idx)    #knn points(B, 512, k, 3)
        knn_x = index_points(x, knn_idx)        #knn points(B, 512, k, 72)
...
class LGA(nn.Module):
    ...
    def forward(self, lc_xyz, lc_x, knn_xyz, knn_x):
        ...
        # Feature Expansion
        B, G, K, C = knn_x.shape
        knn_x = torch.cat([knn_x, lc_x.reshape(B, G, 1, -1).repeat(1, 1, K, 1)], dim=-1)    #(B, G, K, 72*2)
...
class Pooling(nn.Module):
    ...
    def forward(self, knn_x_w):
        ...
        lc_x = knn_x_w.max(-1)[0] + knn_x_w.mean(-1)    #(B, 144, 512)
...
class PosE_Initial(nn.Module):
    ...
    def forward(self, xyz):
        B, _, N = xyz.shape    
        feat_dim = self.out_dim // (self.in_dim * 2)    #out_dim / 6
        
        feat_range = torch.arange(feat_dim).float().cuda()   #m  
        dim_embed = torch.pow(self.alpha, feat_range / feat_dim)    #(12,)
        div_embed = torch.div(self.beta * xyz.unsqueeze(-1), dim_embed)     #(B, 3, 1024, 12)

        sin_embed = torch.sin(div_embed)
        cos_embed = torch.cos(div_embed)
        position_embed = torch.stack([sin_embed, cos_embed], dim=4).flatten(3)      #(B, 3, 1024, 24)
        position_embed = position_embed.permute(0, 1, 3, 2).reshape(B, self.out_dim, N)    #(B, 72, 1024)
...
class PosE_Geo(nn.Module):
    ...
    def forward(self, knn_xyz, knn_x):
        ...
        sin_embed = torch.sin(div_embed)
        cos_embed = torch.cos(div_embed)
        position_embed = torch.stack([sin_embed, cos_embed], dim=5).flatten(4)     #(B, 3, 512, 90, 48)
...
class EncNP(nn.Module):  
    ...
    def forward(self, xyz, x):
        ...
        # Raw-point Embedding
        x = self.raw_point_embed(x)     #(B, 72, 1024)

...
class ContextUnet(nn.Module):
    ...
    def forward(self, x, c, t, context_mask):
        ...
        context_mask = context_mask[:, None]    #(B,) -> (B, 1)
...
class DDPM(nn.Module):
    ...
    def sample(self, n_sample, size, device, guide_w = 0.0):
        ...
        context_mask = torch.zeros_like(c_i).to(device)     #(B,) 取1是掩码
...
if __name__ == "__main__":
    train_mnist()
    unet = ContextUnet(in_channels=1, n_feat=256, n_classes=10)

    x_i = torch.randn(3, 1, 28, 28)
    t_is = 100
    c_i = torch.arange(0,3).repeat(2)
    context_mask = torch.zeros_like(c_i)     #取1是掩码
    context_mask = context_mask.repeat(2)
    context_mask[3:] = 1.
    eps = unet(x_i, c_i, t_is, context_mask)

...
class FeatureExtractor(nn.Module):
    def __init__(self, out_dim=1024, n_knn=20):
        ...
        self.transformer_1 = vTransformer_posenc(128, dim=64, n_knn=n_knn)
        self.sa_module_2 = PointNet_SA_Module_KNN(128, 16, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.transformer_2 = vTransformer_posenc(256, dim=64, n_knn=n_knn)
...
class Denoise(nn.Module):
    ...
    def forward(self, pcd):
        ...
        #retrun (B, N, 3)
        B, N, _ = pcd.shape
        dist_knn = distance_knn(self.nsample, pcd, pcd)   #(B, N, nsample)
        dist_knn = dist_knn.mean(dim=-1)    #(B, N)
        #找到离群点
        idx = torch.argsort(dist_knn, dim=1, descending=True)   #(B, N)
        new_pcd = gather_operation(pcd.transpose(-2, -1).contiguous(), idx.int())  #(B, 3, N)
        new_pcd = new_pcd.transpose(-2, -1)  #(B, N, 3)
        for i in range(B):
...
class SeedFormer_fusion(nn.Module):
    """
    ...
    # radius=1, up_factors=None, seed_factor=2, interpolate='three', attn_channel=True):
        ...
        """
        super(SeedFormer_fusion, self).__init__()
    ...
    def forward_decoder(self, feat, partial_cloud, patch_xyz, patch_feat):
        ...
        for i, layer in enumerate(self.up_layers):
            pcd, _ = layer(pcd, seed, seed_feat, K_prev)    #(B, 3, N)
            if i <= 1:
                pred_pcds.append(pcd.permute(0, 2, 1).contiguous())
                pcd = self.denoise[i](pcd.permute(0, 2, 1).contiguous())
    ...
    def get_loss(self, recon, partial, gt):
        ...
        loss_sum = (cdc + cd1 + cd2 + cd3 + partial_matching + density_loss)
